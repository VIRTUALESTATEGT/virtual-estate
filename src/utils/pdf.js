...
// ── Build full HTML from machote (strips external font links for serverless) ──
function buildCotizacionHTML(cot) {
  ...
  const machoteFile = require('path').join(process.cwd(), 'public', 'assets', 'machote-cotizacion.html');
  let html = require('fs').readFileSync(machoteFile, 'utf8');
  // Strip scripts; strip Google Fonts links (block networkidle in serverless)
  ...
  const dataScript = `<script>(function(){
    ...
    if(cv[0])cv[0].textContent=${JSON.stringify(nombreCompleto)};
    ...
    var firmas=document.querySelectorAll(".firma-nom");if(firmas[1])firmas[1].textContent=${JSON.stringify(nombreCompleto)};
  })();<\/script>`;
...
async function generarCotizacionPDFFallback(cot) {
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.text(`Fecha: ${fecha}     Cliente: ${nombreCompleto}`);
...
async function subirPDFSupabase(pdfBuffer, filename) {
  const { error } = await supabase.storage
    ...
    .upload(`cotizaciones/${filename}`, pdfBuffer, { contentType: 'application/pdf', upsert: true });
