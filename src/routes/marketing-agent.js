...
// API para el Agente IA de contenido para redes sociales
...
// ============================================================
// GENERATED POSTS — list / approve / reject / publish
// ============================================================
...
// ============================================================
// GENERATE — call Claude to create post drafts
// ============================================================
...
router.post('/generate', async (req, res) => {
  try {
    ...
    const prompt = buildGenerationPrompt(brand, instructions, orders);
...
- Horarios de publicación: ${(instructions?.publish_times || ['09:00','13:00','18:00']).join(', ')}

ÓRDENES ACTIVAS:
${orders?.map(o => `- [Prioridad ${o.priority}] ${o.instruction}`).join('\n') || '- Genera contenido inmobiliario general de Virtual Estate GT'}

Genera exactamente 5 posts para redes sociales. Para cada post incluye TODOS estos campos:
- theme: tema corto del post
- content: texto principal del post (español)
- instagram_caption: caption optimizado para Instagram (máx 2200 chars)
- facebook_caption: caption para Facebook (puede ser más largo)
- hashtags: array de hashtags relevantes
- image_description: descripción detallada de la imagen ideal para este post
...
async function publishToInstagram(post) {
  ...
  const { data: container } = await axios.post(`${base}/media`, {
    image_url:    post.image_url,
    caption,
    access_token: ACCESS_TOKEN
  });

  const { data: published } = await axios.post(`${base}/media_publish`, {
